...
uint8_t svl_packet_wait(svl_packet_t *packet)
{
  ...
  {
    uint32_t indi = 0x00;
    for (indi = 0; indi < packet->pl_len; indi++)
    { // Fill payload with data
      svl_packet_read_byte((packet->pl + indi));
    }
    for (; indi < packet->max_pl_len; indi++)
...
uint16_t svl_packet_get_crc16(svl_packet_t *packet)
{
  ...
  for (uint16_t indi = 0; indi < packet->pl_len + 2 + 1; indi++)
  { // addtl 2 bytes for padding, 1 byte to process cmd
